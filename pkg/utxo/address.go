...
func convertBits(data []byte, fromBits, toBits int, pad bool) []byte {
	...
	if pad && bits > 0 {
		result = append(result, byte(acc<<(toBits-bits)))
	}
...
func bech32mChecksum(hrp string, data []byte) []byte {
	...
	// Polynomial coefficients for bech32m
	// Generator polynomial: x^6 + x^3 + x + 1
	...
	for i := 0; i < len(data)+6; i++ {
		...
		for j := 0; j < 6; j++ {
			if (top>>j)&1 == 1 {
				values[i+6-j] ^= 0x98 >> (5 - j) & 1
			}
		}
	}

	// More complex polynomial computation for bech32m
	polymod := bech32mPolymod(append(expandHrp(hrp), data...)) ^ 1
	result := make([]byte, 6)
	for i := 0; i < 6; i++ {
