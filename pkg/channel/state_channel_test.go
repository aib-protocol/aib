...
// ============================================================================
// UpdateState and CloseChannel Tests
// ============================================================================
...
func TestManager_UpdateState(t *testing.T) {
	...
	// Create new state with higher sequence
	...
	updatedState, err := manager.UpdateState(channel, newState)
	if err != nil {
		t.Fatalf("UpdateState failed: %v", err)
	}

	if updatedState.Sequence != 1 {
		t.Errorf("Sequence = %d, expected 1", updatedState.Sequence)
	}

	// Verify channel state was updated
	updatedChannel, _ := manager.GetChannel(channel.ID)
	if updatedChannel.Sequence != 1 {
		t.Errorf("Channel sequence = %d, expected 1", updatedChannel.Sequence)
	}
}

func TestManager_UpdateState_ChannelNotFound(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 24 * time.Hour,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	_, err := manager.UpdateState(nonExistentChannel, newState)
	if err == nil {
		t.Error("Should fail with non-existent channel")
	}
}

func TestManager_UpdateState_InvalidSequence(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 24 * time.Hour,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	// Create state with same sequence (should fail)
	newState := interfaces.SignedState{
		ChannelID: channel.ID,
	...
	_, err := manager.UpdateState(channel, newState)
	if err == nil {
		t.Error("Should fail with invalid sequence")
	}
}

func TestManager_UpdateState_ChannelClosed(t *testing.T) {
	cfg := &Config{
	...
	// Wait and finalize
	time.Sleep(2 * time.Millisecond)
	manager.FinalizeClose(channel.ID)
...
func TestManager_UpdateState_InvalidBalance(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 24 * time.Hour,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	// Create state with changed total balance (invalid)
	newState := interfaces.SignedState{
		ChannelID: channel.ID,
		Sequence:  1,
	...
	_, err := manager.UpdateState(channel, newState)
	if err == nil {
		t.Error("Should fail with invalid balance (total changed)")
	}
}

func TestManager_UpdateState_InvalidSignature(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 24 * time.Hour,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	// Create state signed by wrong key
	newState := interfaces.SignedState{
		ChannelID: channel.ID,
		Sequence:  1,
		BalanceA: 4000,
	...
	newState.SigA = ed25519.Sign(wrongPrivKey, stateData) // Wrong key
	newState.SigB = ed25519.Sign(tp.privKeyB, stateData)
...
func TestManager_CloseChannel(t *testing.T) {
	...
	err := manager.CloseChannel(ctx, channel, finalState)
	if err != nil {
...
func TestManager_CloseChannel_ChannelNotFound(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 24 * time.Hour,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	err := manager.CloseChannel(context.Background(), nonExistentChannel, finalState)
	if err == nil {
		t.Error("Should fail with non-existent channel")
	}
}

func TestManager_CloseChannel_AlreadyClosed(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 1 * time.Millisecond,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	tp.signState(&finalState)
	manager.CloseChannel(ctx, channel, finalState)

	// Try to close again
	secondState := interfaces.SignedState{
		ChannelID: channel.ID,
		Sequence:  2,
		BalanceA:  3000,
		BalanceB: 5000,
	...
	err := manager.CloseChannel(ctx, channel, secondState)
	if err == nil {
		t.Error("Should fail when channel already closed")
	}
}

func TestManager_CloseChannel_ChannelInDispute(t *testing.T) {
	cfg := &Config{
		ChallengePeriod: 24 * time.Hour,
		MinDeposit:      1000,
		MaxChannelValue: 1000000,
	...
	// Try to close channel in dispute
	finalState := interfaces.SignedState{
		ChannelID: channel.ID,
		Sequence:  3,
		BalanceA:  4000,
	...
	err := manager.CloseChannel(ctx, channel, finalState)
	if err == nil {
		t.Error("Should fail when channel in dispute")
	}
}
