...
func TestPriceOracle_InvalidateAllCache(t *testing.T) {
	config := DefaultConfig()
	...
	// Populate cache
	oracle.GetPrice(PairBTCUSD)
	oracle.GetPrice(PairETHUSD)

	// Invalidate all
	oracle.InvalidateAllCache()

	// GetCachedPrice should fail
	_, err := oracle.GetCachedPrice(PairBTCUSD)
	if err == nil {
		t.Error("should error after invalidating cache")
	}
}

func TestPriceOracle_GetCachedPrice(t *testing.T) {
	config := DefaultConfig()
	config.CacheTTL = 1 * time.Hour
	...
	_, err := oracle.GetCachedPrice(PairBTCUSD)
	if err == nil {
		t.Error("should error when no cache exists")
	}
	...
	// Should work now
	cached, err := oracle.GetCachedPrice(PairBTCUSD)
	if err != nil {
		t.Fatalf("GetCachedPrice failed: %v", err)
	}
	if cached.Price != 50000.0 {
		t.Errorf("expected cached price 50000, got %f", cached.Price)
	}
}
